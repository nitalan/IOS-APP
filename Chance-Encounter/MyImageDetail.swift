...
class MyImageDetail: UIViewController,UITableViewDataSource,UITableViewDelegate{
    ...
    let commentCellIdentifier = "MyCommentCellImage"
    ...
    @IBOutlet weak var MyImageDetail: UIImageView!
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCellWithIdentifier(commentCellIdentifier, forIndexPath: indexPath) as! MyImageCell
        ...
        cell.MyCommentTextImage.text = comment.comment_content
        
        let myString = comment.comment_time
        var myArray2 = myString.componentsSeparatedByString(".")
        cell.MyTimeImage.text = myArray2[0]
    ...
    override func viewWillAppear(animated: Bool) {
       ...
       // print (content);
        ...
        NSURLConnection.sendAsynchronousRequest(request, queue:NSOperationQueue.mainQueue(), completionHandler: { (request:NSURLResponse?,data:NSData?,error:NSError?) -> Void in
            ...
            if error==nil{
                self.MyImageDetail.image = UIImage(data:data!)
            }  })
